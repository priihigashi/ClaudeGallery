...
async function verifyLive(){
  const names=['real-estate-study.html','real-estate-schools.js','cameron-bank.enc.json','cameron-section-3.enc.json','cameron-section-4.enc.json','real-estate-schools-sections.js','real-estate-quiz.html'];
...
async function main(){
  const filename='cameron-bank.enc.json',saved=fs.readFileSync(filename),extraSaved=fs.readFileSync('cameron-section-3.enc.json'),fourSaved=fs.readFileSync('cameron-section-4.enc.json');
  ...
  const extraQuestions=['Q903','Q904'].map((id,i)=>({...question(id),sectionId:'FLREEPS3',sourceQuestion:i+1,sourcePdfPages:[1]}));
  const extra={version:1,school:'cameron',section:{id:'FLREEPS3',number:3,title:'Section 3: Sales Associate (Unit 4) and Brokers (Unit 10)',total:2},questions:extraQuestions,pending:[]};
  const extraIV=crypto.randomBytes(12),extraCipher=crypto.createCipheriv('aes-256-gcm',key,extraIV);
  const extraEncrypted=Buffer.concat([extraCipher.update(zlib.gzipSync(Buffer.from(JSON.stringify(extra)))),extraCipher.final(),extraCipher.getAuthTag()]);
  fs.writeFileSync('cameron-section-3.enc.json',JSON.stringify({version:1,algorithm:'AES-GCM',compression:'gzip',iv:extraIV.toString('base64'),ciphertext:extraEncrypted.toString('base64')}));
  const four={version:1,school:'cameron',section:{id:'FLREEPS4',number:4,title:'Section 4: Synthetic fixture',total:2},questions:['Q905','Q906'].map((id,i)=>({...question(id),sectionId:'FLREEPS4',sourceQuestion:i+1,sourcePdfPages:[1],sourceWarning:i===0?'The course explanation ends mid-sentence in this synthetic fixture.':undefined})),pending:[],sourceAttempts:[{sectionId:'FLREEPS3',correct:1,total:2,duration:'00:00:27',duplicateQuestions:2,importAsAppProgress:false},{sectionId:'FLREEPS4',correct:0,total:2,duration:'00:02:15',importAsAppProgress:false}]};
  const fourIV=crypto.randomBytes(12),fourCipher=crypto.createCipheriv('aes-256-gcm',key,fourIV);
  const fourEncrypted=Buffer.concat([fourCipher.update(zlib.gzipSync(Buffer.from(JSON.stringify(four)))),fourCipher.final(),fourCipher.getAuthTag()]);
  fs.writeFileSync('cameron-section-4.enc.json',JSON.stringify({version:1,algorithm:'AES-GCM',compression:'gzip',iv:fourIV.toString('base64'),ciphertext:fourEncrypted.toString('base64')}));
  let browser,server;
  try{
    ...
    await frame.waitForFunction(()=>window.RE_BANK_COUNT===6);await frame.locator('#sectionSelect').selectOption('FLREEPS1');assert.equal(await frame.locator('#qTotal').innerText(),'2');
    ...
    await frame.locator('#resumeBar button').first().click();assert.equal(await frame.locator('#nRight').innerText(),'1');assert.equal(await frame.locator('#sectionSelect').inputValue(),'FLREEPS1');assert.equal(await frame.locator('.opt:disabled').count(),4);
    ...
    await frame.locator('#sectionSelect').selectOption('FLREEPS3');
    assert.equal(await frame.locator('#qTotal').innerText(),'2');assert(await frame.evaluate(()=>order.every(q=>q.sectionId==='FLREEPS3')));
    assert((await frame.locator('#qSrc').innerText()).includes('Source question'));
    await frame.locator('[data-k="a"]').click();
    assert((await frame.locator('#fb').innerText()).includes('Additional clarification based on this source.'));
    assert.equal(await frame.locator('[data-bk]').count(),0);
    await page.screenshot({path:'school-test-screenshots/section3-fixture-mobile.png',fullPage:true});
    await page.locator('#homeBtn').click();await page.locator('#cameron').click();frame=await (await page.locator('#quizFrame').elementHandle()).contentFrame();
    await frame.locator('#resumeBar button').first().click();assert.equal(await frame.locator('#sectionSelect').inputValue(),'FLREEPS3');assert.equal(await frame.locator('#nRight').innerText(),'1');assert.equal(await frame.locator('.opt:disabled').count(),4);
    assert(await frame.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
    await frame.locator('#sectionSelect').selectOption('FLREEPS4');assert.equal(await frame.locator('#qTotal').innerText(),'2');
    assert(await frame.evaluate(()=>order.every(q=>q.sectionId==='FLREEPS4')));
    const historyBeforeSource=await frame.evaluate(()=>JSON.stringify(loadJ('reqz_cameron_hist',{})));
    await frame.locator('#statsBtn').click();assert((await frame.locator('#sourceAttempts').innerText()).includes('separate from app progress'));
    assert.equal(await frame.evaluate(()=>JSON.stringify(loadJ('reqz_cameron_hist',{}))),historyBeforeSource);
    ...
    await page.locator('#homeBtn').click();await page.locator('#cameron').click();frame=await (await page.locator('#quizFrame').elementHandle()).contentFrame();
    await frame.locator('#resumeBar button').first().click();assert.equal(await frame.locator('#sectionSelect').inputValue(),'FLREEPS4');assert.equal(await frame.locator('.opt:disabled').count(),4);
    await frame.evaluate(()=>reviewQ('ca-Q905'));assert((await frame.locator('#rvBody .ca-source-warning').innerText()).includes('mid-sentence'));
    ...
    await frame.locator('#sectionSelect').selectOption('all');assert.equal(await frame.locator('#qTotal').innerText(),'6');
    assert.equal(await page.evaluate(()=>localStorage.getItem('reqz_hist')),sentinel);
    await page.goto(address);await page.locator('#cameron').click();frame=await (await page.locator('#quizFrame').elementHandle()).contentFrame();await frame.waitForFunction(()=>window.RE_BANK_COUNT===6);
  ...
  }finally{fs.writeFileSync(filename,saved);fs.writeFileSync('cameron-section-3.enc.json',extraSaved);fs.writeFileSync('cameron-section-4.enc.json',fourSaved);if(browser)await browser.close();if(server)await new Promise(r=>server.close(r));}
