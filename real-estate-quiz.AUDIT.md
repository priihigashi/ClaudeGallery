...
## Feature backlog
- **Cross-device sync (phone ↔ laptop)** (Priscila request, 2026-06-22): make progress follow her across devices, no login (single user). *Design decisions:* build TRUE two-way sync (not the existing write-only `SYNC_URL` hook) — on answer, POST to a Google Apps Script web app that updates a Google Sheet; on load, GET + merge remote into local. *Sheet shape (answers her "does it clear or accumulate?"):* a **per-question state tab** that updates rows IN PLACE (~270 rows, bounded — does NOT grow forever) + a **sessions tab** (one row per round, capped). So it's compact, not an ever-growing dump. *Where it lives:* in the Real Estate Studies project area, and registered in the Spreadsheet Hub per the global rule — tracked, not clutter. Status: queued (do after the search box).
